...
export default function Storytelling() {
  ...
  // Clean, high-contrast stroke colors for waveforms on light screen
  const getStrokeColors = (idx: number) => {
    switch (idx) {
      case 0:
        return { w1: "stroke-indigo-600", w2: "stroke-rose-500/50", w3: "stroke-indigo-400/25" };
      case 1:
        return { w1: "stroke-cyan-600", w2: "stroke-teal-500/50", w3: "stroke-cyan-400/25" };
      case 2:
        return { w1: "stroke-emerald-600", w2: "stroke-emerald-500/50", w3: "stroke-emerald-400/25" };
      case 3:
      default:
        return { w1: "stroke-violet-600", w2: "stroke-indigo-500/50", w3: "stroke-violet-400/25" };
    }
  };
  ...
  return (
    <section id="ciencia" className="py-24 relative overflow-hidden bg-[#F8FAFC] border-b border-slate-200/80">
      
      {/* Dynamic subtle ambient glow */}
      <div
        style={{
          boxShadow: `0 0 120px 40px ${modules[activeIndex].glowColor}`,
          transition: "box-shadow 0.8s cubic-bezier(0.16, 1, 0.3, 1)",
        }}
        className="absolute top-1/2 left-1/3 w-72 h-72 rounded-full bg-transparent opacity-40 pointer-events-none blur-3xl"
      />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-16">
          ...
          <div className="w-full lg:w-1/2 flex items-center justify-center relative">
            ...
            <div className="w-full max-w-[340px] md:max-w-[440px] aspect-[4/3] rounded-2xl bg-white border border-slate-200/90 shadow-[0_15px_40px_-5px_rgba(0,0,0,0.06)] p-4 flex flex-col relative overflow-hidden backdrop-blur-xl">
              {/* Top Bar with scan dots */}
              <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-2 relative z-20">
                <div className="flex items-center gap-2">
                  <span className="relative flex h-2 w-2">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-500 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-2 w-2 bg-indigo-600"></span>
                  </span>
                  <span className="text-[10px] font-extrabold tracking-widest text-slate-600 uppercase font-mono">
              ...
              <div className="flex-1 bg-slate-950/[0.02] rounded-xl relative border border-slate-100 overflow-hidden">
                {/* Digital Grid Background */}
                <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg">
                  <rect width="100%" height="100%" fill="url(#oscilloscope-grid)" />
                  <line x1="0" y1="150" x2="400" y2="150" stroke="rgba(79, 70, 229, 0.08)" strokeDasharray="3,3" strokeWidth="1" />
                </svg>
                ...
                <svg viewBox="0 0 400 300" className="w-full h-full relative z-10">
                  ...
                  <g className="font-mono font-bold text-[9px] fill-slate-500 select-none">
                ...
                {/* CRT subtle reflection */}
                <div className="absolute inset-0 pointer-events-none bg-gradient-to-b from-white/0 via-white/10 to-white/0 opacity-40 animate-[pulse_4s_infinite]" />
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col items-center justify-center">
              ...
              {modules.map((mod, idx) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`absolute inset-0 w-full h-full max-w-[340px] md:max-w-[420px] rounded-2xl border border-slate-200/90 bg-white/95 backdrop-blur-xl p-6 md:p-8 flex flex-col justify-between shadow-[0_12px_36px_-6px_rgba(0,0,0,0.06)] cursor-pointer group hover:border-indigo-300 transition-colors duration-300 select-none`}
                  >
                    <div>
                      {/* Card Header */}
                      <div className="flex items-center justify-between mb-5">
                        ...
                        <div className="w-10 h-10 rounded-lg bg-indigo-50 border border-indigo-100/80 flex items-center justify-center">
                          {mod.icon}
                        </div>
                      </div>
                    ...
                    <div>
                      <div className="grid grid-cols-3 gap-2 border-t border-slate-100 pt-4 mb-2">
                      ...
                      {/* Hint for front card */}
                      {styles.isFront && (
                        <div className="text-[9px] font-extrabold text-indigo-600 flex items-center justify-end gap-1 uppercase tracking-widest mt-2 group-hover:translate-x-1 transition-transform duration-300">
            ...
            <div className="flex items-center justify-between w-full max-w-[340px] md:max-w-[420px] mt-6 px-1">
              ...
              <button
                onClick={handlePrev}
                className="w-9 h-9 rounded-full border border-slate-200 hover:border-slate-300 bg-white shadow-xs flex items-center justify-center text-slate-600 hover:text-slate-900 hover:scale-105 transition-all duration-300 cursor-pointer"
      ...
      <svg className="hidden">
        <defs>
          <pattern id="oscilloscope-grid" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M 20 0 L 0 0 0 20" fill="none" stroke="rgba(79, 70, 229, 0.05)" strokeWidth="0.8"/>
          </pattern>
        </defs>
      </svg>
    </section>
